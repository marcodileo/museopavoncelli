...
foreach ($ecommerce->getCategoriesTree() as $catId => $cat) {
    if (isset($cat['containsProductsWithProductPage']) && $cat['containsProductsWithProductPage']) {
        $roots[] = $cat;
        if (!isset($_GET['category']) || $catId == $_GET['category']) {
            $products = array_merge($products, getProducts($cat, '', isset($cat['name']) ? $cat['name'] : ''));
        }
    }
}
...
function getProducts($cat, $prefix, $catPath) {
    ...
    foreach ($cat['products'] as $prodId) {
        ...
        if (isset($prod['productPageLinkType']) && $prod['productPageLinkType'] == 'productpage') {
    ...
    foreach ($cat['categories'] as $idSubCat => $subCat) {
        if (isset($subCat['containsProductsWithProductPage']) && $subCat['containsProductsWithProductPage']) {
