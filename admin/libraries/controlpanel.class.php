...
class ControlPanel {
    ...
    public function getMainTemplate()
    {
        ...
        if (
            isset(Configuration::getCart()->getCommentsData()['enabled']) && 
            Configuration::getCart()->getCommentsData()['enabled'] && 
            isset(Configuration::getCart()->getCommentsData()['type']) &&
            Configuration::getCart()->getCommentsData()['type'] == 'websitex5'
        ) {
            $menu[] = array(
        ...
        if (
            isset($settings['dynamicobjects']) && 
            (
                (isset($settings['dynamicobjects']['pages']) && count($settings['dynamicobjects']['pages'])) || 
                (isset($settings['dynamicobjects']['template']) && count($settings['dynamicobjects']['template']))
            )
        ) {
            $menu[] = array(
