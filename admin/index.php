...
if (isset($imSettings['ecommerce']) && isset($imSettings['ecommerce']['database'])) {
    ...
    if ($ecommerce->isDbConnected()) { // Connection check
        ...
        if (
            isset($ecommerce->getCommentsData()['enabled']) && $ecommerce->getCommentsData()['enabled'] &&  
            isset($ecommerce->getCommentsData()['type']) && $ecommerce->getCommentsData()['type'] == 'websitex5'
        ) { 
            ...
            foreach ($cartComments as $productId => $productCommentsData) {
                if (isset($productCommentsData['comments'])) {
